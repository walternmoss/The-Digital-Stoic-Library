...
1. Run both `fetch_*.py` scripts.
2. Run both `translate_*.py` scripts.
3. Run both `*_lexical_tagging.py` scripts.
4. Run both `clean_*.py` scripts.

### 4. Viewing the Interactive Library

The project results are viewed through a web browser. Because the HTML files use the `fetch()` command, you must host them on a local server.
